...
//! BFS launch planner over the read-only dependency scan (§7.3).
//!
//! The planner walks the real `DT_NEEDED` graph from the launch root: open
//! each file, freeze its `FileSnapshotId`, scan it with the loader's
//! [`scan_artifact`] (no allocation, no copy), classify every dependency
//! request through the namespace rules (§4), and record the images and edges
//! of the whole closure. The output [`NamespaceLoadPlan`] is a transient
//! object describing *this one launch* as observed from the VFS — it is not a
//! package manifest and nothing is written to firmware.
//!
//! Its purpose (§7.1): the launch path sees the complete system closure
...
use crate::{
    application::{
        adapters::{
            resolver::identity_from_snapshot,
...
/// One planned image: the resolved path, the frozen snapshot it was scanned
/// under, and what the scan saw (§7.3).
pub struct PlannedImage {
    /// The normalized absolute path this image was opened at.
    path: String,
    /// The snapshot frozen when the file was opened; the real load must use
    /// the same reader or re-open and verify an identical snapshot (§7.4).
    snapshot: FileSnapshotId,
    /// The artifact identity derived from the same snapshot (the graph dedup
    /// key, identical to the identity the real load will produce).
    identity: ArtifactIdentity,
    /// The image's role: `ExecutableRoot` for the launch root, `SharedObject`
...
impl PlannedImage {
    ...
    /// The frozen VFS snapshot (§7.4).
    pub fn snapshot(&self) -> FileSnapshotId {
        self.snapshot
    }

    /// The artifact identity from the same snapshot.
    pub fn identity(&self) -> &ArtifactIdentity {
        &self.identity
    }

    /// The image's role in the link session.
...
/// One dependency edge: requester → provider (§7.3).
...
/// deduplicated system catalog keys the launch will batch-acquire (§7.3).
pub struct NamespaceLoadPlan {
    /// The index of the launch root (always 0).
    root: usize,
...
impl NamespaceLoadPlan {
    ...
    /// the batch-acquire input (§8).
    pub fn system_keys(&self) -> &[DependencyName] {
        &self.system_keys
    }
}

/// The BFS planner (§7.3): scan the real dependency closure of one launch.
...
impl<'a> NamespaceLoadPlanner<'a> {
    ...
    /// Walk the closure from the namespace's root path (§7.3).
    ///
    /// Each file is opened exactly once and scanned read-only; every
    /// discovered image carries the `FileSnapshotId` and identity the real
    /// load must agree with (§7.4).
    pub fn plan(&self) -> LoadResult<NamespaceLoadPlan> {
        ...
        // Seed the root: an exact open, no fallback (§3.2). The BFS queue
        ...
        while queue_head < queue.len() {
            ...
            // against the requesting ELF's directory (§4.2).
    ...
    /// scanning the file on first sight (§4).
    ///
    ...
    fn resolve_request(
        ...
        // namespace (§4.3): its relocations must be identical no matter
        ...
        // present-but-unloadable file fails without fallback (§4.3).
    ...
    /// Open `path`, freeze its snapshot and scan it read-only (§7.3 steps
    /// 1–2). `role` is `ExecutableRoot` for the launch root and
    ...
    fn open_and_scan(&self, path: &str, role: ArtifactRole) -> LoadResult<PlannedImage> {
        ...
        // whatever DT_SONAME it carries (§4.1 rule 2).
        let entry = if role == ArtifactRole::SharedObject {
            self.system_catalog.resolve_path(path)
        } else {
            None
        };
        let identity = identity_from_snapshot(snapshot, entry.and_then(|entry| entry.build_id));
        Ok(PlannedImage {
            path: path.to_string(),
            snapshot,
            identity,
    ...
    /// (§4.3: only `ENOENT`/`ENOTDIR` advance the search).
...
mod tests {
    ...
    /// The test catalog mirrors the boot runtime's (§6): two system DSOs
    ...
    fn dso_elf(needed: &[&str], soname: Option<&str>) -> Vec<u8> {
        ...
        bytes[28..32].copy_from_slice(&(ELF32_EHDR_SIZE as u32).to_le_bytes()); // e_phoff
                                                                                // e_flags: EABI5 | EF_ARM_ABI_FLOAT_SOFT, the admission the
                                                                                // arm_thumb_soft_float profile enforces.
        ...
        bytes[44..46].copy_from_slice(&2u16.to_le_bytes()); // e_phnum
                                                            // PT_LOAD: r-x, covers the whole file at vaddr 0x1000. ELF32 phdr
        ...
        bytes[ph0 + 28..ph0 + 32].copy_from_slice(&4u32.to_le_bytes()); // p_align
                                                                        // PT_DYNAMIC: same ELF32 field order.
        let ph1 = ph0 + 32;
    ...
    fn namespace_at(pwd: &str, launch: &str) -> ApplicationNamespace {
        ApplicationNamespace::from_launch_path(
            ...
            blueos_loader::LinkDomainId::new(1),
        )
        .expect("namespace")
    }
    ...
    fn plan_walks_private_then_system_closure() {
        ...
        // The scanned SONAME came through (§7.2).
        assert_eq!(
            plan.images()[0]
                .scanned()
                .declared_soname
    ...
    /// A plain-name request resolved nowhere is unresolved (§4.3).
    #[test]
    ...
    /// The same DSO requested twice is planned once (§7.3 dedup).
    #[test]
    ...
    /// VFS path produce two graph edges but only one mapped image (§5.2).
    #[test]
