...
use crate::{
    ...
    DynamicSegmentInfo, ElfClass, Endian, ErrorContext, ImageLoadTransaction, ImageMemory,
    LoadError, LoadErrorKind, LoadResult, LoadStage, MappedImage, MemoryPermissions, TargetAddr,
    TargetLocation,
};
...
impl MappedImage {
    pub fn decode_runtime<M: ImageMemory>(
        self,
...
fn decode_dynamic_tags<M: ImageMemory>(
    ...
    for index in 0..entry_count {
        ...
        let current = location.checked_add(index.checked_mul(entry_size).ok_or_else(|| {
            LoadError::new(
                LoadStage::Metadata,
        ...
        memory.read(current, &mut raw[..entry_size as usize])?;
...
fn accept_dynamic_tag(tags: &mut DynamicTags, tag: u64, value: u64) -> LoadResult<()> {
    match tag {
        ...
        | DT_VERNEEDNUM | DT_TLSDESC_PLT | DT_TLSDESC_GOT => Err(unsupported_dynamic(tag, value)),
        ...
        | DT_BIND_NOW | DT_FLAGS | DT_FLAGS_1 | DT_GNU_HASH | DT_RELACOUNT | DT_RELCOUNT => Ok(()),
        _ => Err(unsupported_dynamic(tag, value)),
    }
}
...
fn decode_relocation_table<M: ImageMemory>(
    ...
    let location = mapped.locate_vaddr(table_vaddr, byte_len, MemoryPermissions::READ)?;
    memory.validate_access(location, byte_len, MemoryPermissions::READ)?;
    let mut raw = [0; 24];
    for index in 0..count {
        let entry_location = location.checked_add(index * entry_size)?;
        memory.read(entry_location, &mut raw[..entry_size as usize])?;
