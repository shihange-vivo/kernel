...
//! Read-only dependency scan (namespace-replacement plan §7.2).
//!
...
//! header, program headers and the S1 dynamic feature summary, and the shared
...
//!
//! The scan output feeds the BFS launch planner (§7.3) and the runtime system
//! batch acquire (§8): knowing the whole system closure up front lets one
...
use crate::{
    ...
    identity::{LoadLimits, LoadProfile, LoadRequest, PHASE05_LOAD_POLICY},
    image::{image_loader::ImageLoader, map::decode_dependency_name_at},
    reader::ElfReader,
};

/// What one artifact's dynamic table declares (§7.2).
///
...
/// Scan one artifact's metadata without any target allocation (§7.2).
///
...
/// program-header structure, phase policy, `PT_DYNAMIC` presence for a shared
/// object, and the S1 tag checks. Only the *resolved* names are produced; no
...
pub fn scan_artifact<R: ElfReader>(
    ...
    let inspected = ImageLoader::new(ReadRef::new(reader), request)
        ...
        .inspect_with_policy(PHASE05_LOAD_POLICY)
        .with_role(role)
        .inspect()
        .map_err(|error| error.at_stage(LoadStage::Discover))?;
    ...
    let Some(dynamic) = dynamic else {
        // A root without a dynamic table: no SONAME, no NEEDED (§7.2).
        return Ok(ScannedArtifact::default());
    };

    // Resolve the raw offsets against the file-backed `.dynstr` using the S1
    // summary's DT_STRTAB/DT_STRSZ — the same pairing rules the S4 decode
    ...
    let (strtab, strsz) = match (summary.strtab(), summary.strsz()) {
        ...
        _ => {
            ...
            // cannot resolve them: the same BadElf the S4 decode reports.
...
/// (S4); the scanner locates the same vaddr in the load segment's file range
